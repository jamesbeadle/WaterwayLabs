<script lang="ts">
  ...
  // Define valid project names type
  type ProjectName = 
    | "OpenFPL" 
    | "Football God" 
    | "GolfPad" 
  ...
  async function loadProjects() {
    try {
      ...
      projects = projectDTOs.map(dto => ({
        ...dto,
        component: getComponentByName(dto.name),
        buttonText: "Visit Site",
        backgroundImage: `/images/${dto.name.toLowerCase()}-background.png`,
        previewImage: `/images/${dto.name.toLowerCase()}-preview.png`,
        mobilePreviewImage: `/images/${dto.name.toLowerCase()}-mobile.png`,
        translateX: '0px',
        twitter: dto.socialLinks.find(([platform]) => platform === 'X')?.[1] || '',
        backgroundColor: dto.mainColour,
        status: getStatusString(dto.status), // Convert status to string
      }));
      
      projectStore.setProjects(projectDTOs);
      selectedProject = projects.find(p => p.id === 3) || projects[0];
      updateGlobalColor(selectedProject?.mainColour ?? '#2CE3A6');
    } catch (error) {
      ...
      <div class="flex-row items-start hidden min-h-screen px-10 pt-20 overflow-x-hidden lg:flex">
        ...
        >
          ...
          <img
            ...
            alt={`${selectedProject?.name} preview`}
            class="absolute z-0 object-contain preview-image-container"
        ...
        <div class="fixed flex flex-col items-center justify-end space-y-4" 
          ...
          {#if selectedProject.socialLinks}
            <a 
              href={selectedProject.socialLinks[0][1]} 
              target="_blank" 
              rel="noopener noreferrer"
              aria-label="Social media link"
              class="block transition hover:opacity-80 relative z-[2]">
